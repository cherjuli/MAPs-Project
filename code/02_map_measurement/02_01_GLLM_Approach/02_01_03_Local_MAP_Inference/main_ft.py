...
import time
import torch
from utils_ft import load_config, set_up_logging, initialize_GLLM_pipeline, run_batched_pipeline_interference, extract_json
import yaml
...
def main():
    ...
    fine_tuned_model_dir = config["fine_tuned_model_dir"]
    ...
    if start_index == 0:
        ...
        shard.to_pandas().to_pickle(os.path.join(datasets_dir, f"{dataset_name}_FT_shard_{args.shard_index}.pkl"))
        ...
        with open(os.path.join(datasets_dir, f"{dataset_name}_FT_shard_{args.shard_index}.pkl"), "rb") as f:
            shard = pickle.load(f)
    ...
    generate_text_pipeline = initialize_GLLM_pipeline(fine_tuned_model_dir=fine_tuned_model_dir,
